...
$macb = $_SESSION['macb'];
$dothoc = $_REQUEST['txtKhoaKinhPhiTHTN'];
$hk = $_REQUEST['h'];
...
if ($n>0)
{
	...
	<div align='center'><h2>Kinh Phí Thực Hành - Thí Nghiệm <br/>Học kỳ $hk</h2></div>		
		...
		<div style='margin-bottom:20px;'>
			<table id='tableKinhPhiTHTN' name='tableKinhPhiTHTN' width='100%' border='0'  cellspacing='0' class='ui-widget ui-widget-content ui-corner-top tableData' >
			<thead>
			  <tr  class='ui-widget-header heading' style='height:20pt; font-weight:bold'>
				<td align='center' title='Mã Số Môn Học' class='ui-corner-tl'>MSMH</td>
	...
	for ($i = 0; $i < $n; $i++)
	{
		if ($resDM["TONG_CONG"][$i]>0){
			...
			if ($ma_mh != $resDM['MA_MH'][$i]){
				echo "<td  align='center'>" .$resDM['MA_MH'][$i]."</td><td >".$resDM["TEN"][$i]."</td>";
				$ma_mh = $resDM['MA_MH'][$i];
			}
			else
	...
	echo "Không có Kinh phí thực hành thí nghiệm";

...
$macb = $_SESSION['macb'];
$dothoc = $_POST['phong_txtKhoaQuanLyPhi'];
$hk = $_POST['h'];
$makhoa = $_POST['phong_txtMaKhoaQuanlyphi']; //base64_decode($_SESSION["makhoa"]);
$tenkhoa = $_POST['tkhoa']; //$_SESSION["tenkhoa"];

$wherekhoa = "and  k.ma_khoa = '$makhoa'";
if ($makhoa==''){
	$wherekhoa = "";
}else{
...
$sqlstr="
select hinh_thuc_dao_tao, ten_khoa,  decode(hinh_thuc_dao_tao, 'CQ', 100, 'DT', 50, 'CD', 50, 'LV', 100, 'ON', 200/3, 'KL', 100) tyle, sum(quan_ly_phi) quan_ly_phi, d.ten_quan_ly_phi
...
and q.hinh_thuc_dao_tao = d.ma_quan_ly_phi (+) $wherekhoa
group by ten_khoa, hinh_thuc_dao_tao, d.ten_quan_ly_phi
order by VIET0DAU_NAME(ten_khoa), ten_quan_ly_phi
";
...
{
	echo "
	<div align='center'><h2>Quản lý phí học kỳ $hk<br/>$tenkhoa</h2></div>		
	...
	for ($i = 0; $i < $n; $i++)
	{
		...
		echo "<td align='left'>{$resDM["TEN_KHOA"][$i]}</td>";

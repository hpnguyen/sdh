...
{
	$sqlstr="	SELECT to_char(add_working_days(so_ngay_xu_ly, sysdate), 'dd/mm/yyyy') ngay_tra, don_gia, ghi_chu, n.ho || ' ' || n.ten ho_ten, h.nguoi_giai_quyet
...
else if ($a=='addychvu')
{
	...
	for ($i=0 ; $i < $count; $i++)
	{
		if (!$error)
		{
			...
			$m = str_replace($searchdb, $replacedb,$_POST["mhv$i"]);
			...
			if (oci_execute($stmt))
			{
				...
				if ($magqhv!='')
				{
					...
					values ('$magqhv','$m','$mayc','$noidung', sysdate,to_date('$ngaytra','dd/mm/yyyy'),'$mann',$sl,$phi,'$ghichu','$mangq', 0)"; 
					$stmt = oci_parse($db_conn, $sqlstr);
					/*
...
else if ($a=='refreshdata')
{
	...
	$sqlstr="	SELECT ma_gqhvu, noi_dung_yc, to_char(ngay_tiep_nhan, 'yyyy-mm-dd') ngay_tiep_nhan, to_char(ngay_hen_tra_kq, 'yyyy-mm-dd') ngay_hen_tra_kq, 
				get_last_ghi_chu_hv(ma_gqhvu) ghi_chu, fk_ma_hoc_vien, hv.ho || ' ' || hv.ten ho_ten_hv, ma_bac, 
				...
				WHERE fk_ma_hoc_vien = ma_hoc_vien(+) and hvu.nguoi_giai_quyet = n.id(+) and hvu.NGUOI_TIEP_NHAN = n1.id(+) and hvu.tinh_trang = tt.ma_tinh_trang(+)
	...
	for ($i = 0; $i < $n; $i++)
	{
		...
		$data.= '["<input type=checkbox id=selectYCHV'.$i.' name=selectYCHV'.$i.' value=\''.$resDM["MA_GQHVU"][$i].'\' '.''.'>",
				  "'.$resDM["MA_GQHVU"][$i].'", "'.escapeWEB($resDM["NOI_DUNG_YC"][$i]).'", "'.$resDM["SO_LUONG"][$i].'",
				  ...
				  "'.$resDM["HO_TEN_HV"][$i].'",
				  "'.$resDM["TEN_TINH_TRANG"][$i].'", "'.escapeWEB($resDM["KET_QUA"][$i]).'",
				  ...
				  "'.$resDM["VI_TRI_LUU"][$i].'", "'.$resDM["QUA_TRINH_CHUYEN"][$i].'",
				  "'.escapeWEB($resDM["GHI_CHU"][$i]).'", "'.$resDM["HET_HAN"][$i].'",
				  "'.escapeWEB($resDM["NGAY_TRA_KQ"][$i]).'", "'.$resDM["ID_NGUOI_GIAI_QUYET"][$i].'"
				 ],';

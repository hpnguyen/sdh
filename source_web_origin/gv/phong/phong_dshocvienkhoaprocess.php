...
if ($_REQUEST["act"]=='dshocvien') 
{
	...
	//echo $sqlstr;
	echo "
		<div align='center'><h2>Danh Sách Học Viên Khóa $khoa<br/>Khoa {$resDM["TEN_KHOA"][0]}</h2></div>
...
else if ($_REQUEST["act"]=='dshocvienfile') 
{
	...
	//$fp = fopen($pathfile, 'w');
	//fwrite($fp,  'Mã HV' . chr(9) . 'Họ' . chr(9) . 'Tên' . chr(9) . 'Phái' . chr(9) . 'Ngày Sinh' . chr(9) . 'Nơi Sinh' . chr(9) . 'Ngành' . chr(9) . 'Đã TN'. chr(9) . 'Tổng chỉ tích lũy MH' . chr(9) . 'Trung bình tích lũy MH' . chr(9) . 'TB Toàn khóa' . chr(9) . 'Tổng chỉ toàn khóa' . chr(9) . 'Đợt cấp bằng' .chr(9). 'Thuộc CTĐT' .chr(13).chr(10) ) ;
	...
	for ($i = 0; $i < $n; $i++)
	{
		...
		.chr(9).$resDM["TEN_NGANH"][$i].chr(9).$resDM["TN"][$i]
		.chr(9).$resDM["TONG_TICH_LUY"][$i].chr(9).$resDM["TB_TICH_LUY_MH"][$i]
		.chr(9).$resDM["DIEM_TB_TOAN_KHOA"][$i].chr(9).$resDM["TONG_CHI_TOAB_KHOA"][$i]
		.chr(9).$resDM["DOT_CAP_BANG"][$i].chr(9).$resDM["CTDT_HV"][$i]
		.chr(13).chr(10);
		fwrite($fp, $tmp);*/
		$j=$i+3;

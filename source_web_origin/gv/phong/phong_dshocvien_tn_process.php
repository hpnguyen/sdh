...
if ($_POST["a"]=='dshocvien') 
{
	...
	//echo $sqlstr;
	echo "
		<div align='center'><h2>Danh Sách Học Viên Tốt Nghiệp Đợt $dot $tieudekhoa</h2></div>
...
else if ($_POST["a"]=='dshocvienfile') 
{
	$sqlstr="	SELECT  k.ten_khoa, dot_cap_bang(h.ma_hoc_vien), h.MA_HOC_VIEN, h.HO, h.TEN , dot_cap_bang(h.ma_hoc_vien),
					...
					DECODE(ctdt_loai(h.MA_HOC_VIEN), 1, 'GDMH-KLTN', 3 , 'Nghiên cứu' , 'GDMH-LVThs' )  huongdt
					FROM hoc_vien h, nganh n, bo_mon b, khoa k, dm_tinh_tp t
			WHERE h.ma_nganh = n.ma_nganh AND h.noi_sinh = t.ma_tinh_tp (+) AND n.ma_bo_mon = b.ma_bo_mon AND b.ma_khoa = k.ma_khoa
			$qKhoa
			AND dot_cap_bang(h.ma_hoc_vien) = '$dot' AND ma_bac = 'TH' AND fk_hinh_thuc_dao_tao = 'CQ'
			ORDER BY ten_nganh, h.ho, h.ten";
	...
	$objPHPExcel->getProperties()->setCreator("$usr")
								 ...
								 ->setTitle("DANH SÁCH HỌC VIÊN KHOA {$resDM["TEN_KHOA"][0]} - Đợt $dot")
								 ->setSubject("DANH SÁCH HỌC VIÊN KHOA {$resDM["TEN_KHOA"][0]} - Đợt $dot")
								 ->setDescription("")
								 ->setKeywords("")

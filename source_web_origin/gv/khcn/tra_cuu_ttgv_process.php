...
if ($w == "bm_dsgv_1") 
{
	$sqlstr="select *
			from (select c.ma_can_bo, viet0dau_name(c.ten) ten_khong_dau, viet0dau_name(c.ho) ho_khong_dau, get_thanh_vien(c.ma_can_bo) ho_ten
						...
						and c.ma_bo_mon='$ma_bm' and trang_thai=1
						order by ten_khong_dau, ho_khong_dau) ar
			where rownum < 24";
...
if ($w == "bm_dsgv_2") 
{
	$sqlstr="select c.ma_can_bo, viet0dau_name(c.ten) ten_khong_dau, viet0dau_name(c.ho) ho_khong_dau, get_thanh_vien(c.ma_can_bo) ho_ten
						...
						where c.fk_loai_can_bo = '00'	
						and c.ma_bo_mon='$ma_bm' and trang_thai=1
						order by ten_khong_dau, ho_khong_dau
			";
	$stmt = oci_parse($db_conn, $sqlstr);
	oci_execute($stmt);
	$n = oci_fetch_all($stmt, $resDM);
...
if ($w == "mcb_ttgv") 
{
	...
	$index=1;
?>
  <div align="center" style="margin-top:0px; font-family:Arial,Helvetica,sans-serif; font-size:95%" id="chitietttgv">
    <table width="100%"   cellspacing="0" cellpadding="0" class="ui-corner-all shawdow tableData" >
      ...
      <tr>
        <td colspan=2>
			...
			<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0"  style="font-family:Arial,Helvetica,sans-serif;">

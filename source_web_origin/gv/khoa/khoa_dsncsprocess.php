...
if ($_REQUEST["act"]=='dsncs') 
{
	...
	echo "
	<div align='center'><h2>Danh Sách Nghiên Cứu Sinh<br/>Khóa $khoa</h2></div>
	<div style='margin-bottom:20px;'>
		<table id='khoa_tableDSNCS' name='khoa_tableDSNCS' width='100%' border='0'  cellspacing='0' class='ui-widget ui-widget-content ui-corner-top' >
        <thead>
...
else if ($_REQUEST["act"]=='dsncsfile') 
{
	$sqlstr="	SELECT T.MA_HOC_VIEN, T.HO, T.TEN , 
				DECODE(T.NGAY_SINH, null, NGAY_SINH_KHONG_CHUAN, TO_CHAR(T.NGAY_SINH, 'dd/mm/yyyy')) Ngay_Sinh, 
				DECODE(T.PHAI, 'M', 'Nam ', 'Nữ') PHAI, TEN_TINH_TP NOI_SINH, N.TEN_NGANH, K.TEN_KHOA,
				...
				WHERE T.KHOA = $khoa 
				AND K.MA_KHOA = $makhoa
				AND T.MA_NGANH = N.MA_NGANH and upper(T.MA_BAC) = 'TS' AND T.FK_HINH_THUC_DAO_TAO = 'CQ' 
				AND N.MA_BO_MON = M.MA_BO_MON AND M.MA_KHOA = K.MA_KHOA 
				AND t.noi_sinh = p.ma_tinh_tp (+) 
				ORDER BY K.TEN_KHOA DESC, N.TEN_NGANH ASC, T.ten_eng, t.ho_eng";
	...
	$pathfile = "download/{$usr}_{$today}_{$time}_Khoa{$makhoa}_dsNghienCuuSinhK{$khoa}.xlsx";	
	/** Include PHPExcel */

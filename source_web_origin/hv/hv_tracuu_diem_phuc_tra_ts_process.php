...
if ($type=='sdb_hoten_ngaysinh-diemts')
{
    ...
    oci_execute($oci_n);
	$row = oci_fetch_all($oci_n,$nam);
	oci_free_statement($oci_n);
	$nam_ts = $nam["NAM"][0];
	
	if(strlen($sbd)<>0 or strlen($hoten)<>0)
	{
	 $Strdata = "SELECT so_bao_danh, ho ||' '||ten hoten, ten_nn || decode(ts.ma_nn, 'M', ' - ', '') ten_nn, ly_do_uu_tien, ly_do_mien_nn||' ' ||SO_DIEM_CHUNG_CHI_EN ly_do_mien_nn, ten_nganh, ten_tinh_tp noi_sinh, NVL(to_char(ngay_sinh,'DD/MM/YYYY'), NGAY_SINH_KHONG_CHUAN) NGAY_SINH, dia_chi_lien_lac, dien_thoai_nr, email, DON_VI_CONG_TAC, so_bao_danh, ts.nam, ma_bac, nvl(fk_doi_tuong_uu_tien, '!') uu_tien, diem_chuan
				 FROM thi_sinh_du_thi ts, nganh n, diem_chuan_tuyen_sinh dc, dm_tinh_tp ttp, mon_thi_ngoai_ngu nn, dm_doi_tuong_uu_tien ut
				 WHERE noi_sinh = ma_tinh_tp(+) AND fk_doi_tuong_uu_tien = ma_uu_tien(+) 
				 AND ts.ma_nn = nn.ma_nn(+) AND ts.ma_chuyen_nganh = n.ma_nganh(+) 
				 AND ma_chuyen_nganh = dc.ma_nganh(+) AND ts.nam = dc.nam(+) 
				 AND ( (ngay_sinh_khong_chuan = ".$ngaysinh.") OR (to_char(ngay_sinh, 'ddmmyyyy') = substr('".$ngaysinh."',1,2)||substr('".$ngaysinh."',4,2)||substr('".$ngaysinh."',7,4))) and (so_bao_danh ='" . $sbd . "' OR upper(ho||' '||ten) = upper('" . $hoten . "') OR upper(ho_eng||' '||ten_eng) = upper('" . $hoten . "')) and ts.nam =".$nam_ts . " order by ts.so_bao_danh desc";
	 $Strdata = "
		...
		and ma_chuyen_nganh = dc.ma_nganh(+) and ts.nam = dc.nam(+) and dc.dot = ts.dot
	 ...
	 {
		echo "<div align=center style='margin:0 0 10px 5px; font-size:14px;'><b>KẾT QUẢ CHẤM PHÚC TRA TUYỂN SINH NĂM ". $hocvien["NAM"][0] ." - ĐỢT 2</b></div>";
		...
		$Strdiem = "select to_char(diem_cb, '990.99') diem_cb, to_char(diem_cb_truoc_pt, '990.99') diem_cb_truoc_pt, to_char(diem_cs, '990.99') diem_cs, to_char(diem_cs_truoc_pt, '990.99') diem_cs_truoc_pt, diem_nn, diem_nn_truoc_pt, to_char(tong_diem_cb, '990.99') tong_diem_cb, to_char(tong_diem_nn, 990) tong_diem_nn, decode(tong_CB_CS, 200, ' ', to_char(tong_CB_CS, '990.99')) tong_CB_CS, diem_bvdc from diem_goi_to_thu_ky_phuc_tra where so_bao_danh ='" . $hocvien["SO_BAO_DANH"][0] . "' and nam =".$nam_ts;
		...
		if(oci_fetch_all($oci_pa_diem, $results))
		{
			...
			else 
			{
				...
				if ($results["DIEM_CB_TRUOC_PT"][0]!="")
						echo " (Trước phúc tra: {$results["DIEM_CB_TRUOC_PT"][0]})";
			}
			echo "</td></tr>";
			...
			if ($results["DIEM_CS"][0] < 0)
				...
				echo $results["DIEM_CS"][0];
				
			if ($results["DIEM_CS_TRUOC_PT"][0]!="")
						echo " (Trước phúc tra: {$results["DIEM_CS_TRUOC_PT"][0]})";
			...
			if ($results["DIEM_NN_TRUOC_PT"][0]!="")
			{
				if ($results["DIEM_NN_TRUOC_PT"][0]<0)
					$diem_nn_truoc_pt = 0;
				else 
					$diem_nn_truoc_pt = $results["DIEM_NN_TRUOC_PT"][0];
				
				echo " (Trước phúc tra: $diem_nn_truoc_pt)";
			}
			echo "</td></tr>";
			...
			if ($hocvien["MA_BAC"][0] != 'TH' and $hocvien["MA_BAC"][0] != 'CTS')
			{
				...
				*/
				...
				echo "<tr><td class='ui-widget-header'>Điều kiện trúng tuyển</td>
					<td class=''>- Điểm cơ bản >= 5.0<br>- Điểm cơ sở >= 5.0<br>- Điểm ngoại ngữ >= 50<br>- Điểm xét duyệt hồ sơ >= 5.0<br>";  
				echo "</td></tr>";

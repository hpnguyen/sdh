...
if ($type=='sdb_hoten_ngaysinh-diemts')
{
	...
	if(strlen($sbd)<>0 or strlen($hoten)<>0)
	{
	 $Strdata = "
		...
		and ma_chuyen_nganh = dc.ma_nganh(+) and ts.nam = dc.nam(+) and ts.dot = dc.dot (+)
	 ...
	 {
		echo "<div align=center style='margin:0 0 10px 5px; font-size:14px;'><b>KẾT QUẢ TUYỂN SINH NĂM $nam_ts ĐỢT $dot_ts</b></div>";
		...
		if(oci_fetch_all($oci_pa_diem, $results))
		{
			...
			if ($results["DIEM_NN"][0] < 0) 
				...
				//if ($hocvien["SO_BAO_DANH"][0] < '3000') //Khong xet ket qua nuoc ngoai
					echo "Miễn thi</td></tr>";
				//else
				//	echo "</td></tr>";
			else 

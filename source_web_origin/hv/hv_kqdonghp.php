...
$sqlstr="SELECT to_char(to_date(value, 'dd-MON-yyyy'), 'dd-MON-yy') dot_hoc_hien_tai FROM config WHERE name = 'DOT_HOC'";
$stmt = oci_parse($db_conn, $sqlstr); oci_execute($stmt); $n = oci_fetch_all($stmt, $resDM); oci_free_statement($stmt);
$dot_hoc_hien_tai = $resDM["DOT_HOC_HIEN_TAI"][0];
?>
	...
	<tr>
	  ...
	  <td align=left>
		<select id=kqdhp_txtHK name=kqdhp_txtHK style="font-size:15px;" onChange="kqdhp_updateKQDHP(this.value);">
			...
			for ($i = 0; $i < $n; $i++){
				if ($dot_hoc_hien_tai == $resDM["DOT_HOC"][$i]) {
					echo "<option value='".$resDM["DOT_HOC"][$i]."' selected>" .$resDM["HOC_KY"][$i]. "</option>";
				}else{
...
function kqdhp_updateKQDHP(p_dothoc)
{
	...
	{
		$("#kqdhp_chitiet").html("<div>Chưa có dữ liệu về học phí</div>");
	}
}

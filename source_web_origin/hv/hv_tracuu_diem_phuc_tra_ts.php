...
$oci_n = oci_parse($db_conn,"select value from config where name = 'KHOA_TUYEN_SINH_WEB' or name = 'PHUC_TRA_TRA_CUU' order by name");
oci_execute($oci_n);
$row = oci_fetch_all($oci_n,$config);
oci_free_statement($oci_n);
$nam_ts = $config["VALUE"][0];
$allow = $config["VALUE"][1];
...
?>
	<form id="form_tracuu_DIEM_TS" name="form_tracuu_DIEM_TS" method="post" action="">
	   <table width="100%" border="0" cellspacing="0" cellpadding="5" align=center>
	    <tr><td></td><td colspan=3 style="font-family: arial; font-size:10pt; font-weight: bold">ĐIỂM PHÚC TRA NĂM <?php echo "$nam_ts ĐỢT $dot_ts"; ?></td></tr>
		<tr>
		...
		<tr>
		  ...
		  <td align=left colspan="3" style='font-size:80%'>
			<a id="diem_ts_btn_xem" >&nbsp;Tra điểm</a>&nbsp;<a id="diem_ts_btn_printpreview" >&nbsp;Xem bản In</a>
		  </td>
	   ...
	   <div id="diem_ts_chitiet" style="margin-top:-15px;" align=center></div>
	</form>
<?php
}
else
{
	echo "<div align=center style='font-size:12px; font-weight:bold; color:red'>Hiện tại chưa có điểm phúc tra tuyển sinh, vui lòng quay lại sau.</div>";
}
?>
</div>
...
function diem_ts_updateDiem(p_sbd, p_hoten, p_ngaysinh)
{
	...
	xreq = $.ajax({
	  type: 'POST', data: DataString, dataType: "html", url: 'hv_tracuu_diem_phuc_tra_ts_process.php',

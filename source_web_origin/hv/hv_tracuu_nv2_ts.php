...
oci_execute($oci_n);$row = oci_fetch_all($oci_n,$config);oci_free_statement($oci_n);
$nam_ts = $config["NAM"][0];

$oci_n = oci_parse($db_conn,"select value DOT_TS from config where name = 'DOT_TUYEN_SINH_WEB'");
oci_execute($oci_n);$row = oci_fetch_all($oci_n,$config);oci_free_statement($oci_n);
$dot_ts = $config["DOT_TS"][0];

$oci_n = oci_parse($db_conn,"select value NGUYEN_VONG_TRA_CUU from config where name = 'NGUYEN_VONG_TRA_CUU'");
oci_execute($oci_n);$row = oci_fetch_all($oci_n,$config);oci_free_statement($oci_n);
$allow = $config["NGUYEN_VONG_TRA_CUU"][0];
...
?>
	...
	<form id="form_tracuu_DIEM_TS" name="form_tracuu_DIEM_TS" method="post" action="">
	   <table width="100%" border="0" cellspacing="0" cellpadding="5" align=center>
		...
		<tr><td></td><td colspan=3 style="font-family: arial; font-size:10pt; font-weight: bold">XÉT TUYỂN NGUYỆN VỌNG 2 TUYỂN SINH NĂM <?php echo $nam_ts; ?> ĐỢT <?php echo $dot_ts; ?></td></tr>
		<tr>
...
{
	echo "<div align=center style='font-size:12px; font-weight:bold; color:red'>Chưa có kết quả xét nguyện vọng 2, vui lòng quay lại sau.</div>";
}
?>
</div>
...
function diem_ts_updateDiem(p_sbd, p_hoten, p_ngaysinh)
{
	...
	xreq = $.ajax({
	  type: 'POST', data: DataString, dataType: "html", url: 'hv_tracuu_nv2_ts_process.php',
